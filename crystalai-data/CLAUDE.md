...
- **No spglib.** Space groups are read from the source, never re-derived: ICSD from the CIF `_space_group_IT_number` (fall back to the H-M symbol via `hm_symbol_to_sg_number` only when the IT number is absent); MP-20 from the CSV field. Crystal system is derived from the space group by the range partition in `sg_to_cs`, making SG/CS disagreement impossible by construction. No symmetry re-analysis is performed.
...
`ase`, `Pysimxrd`, `mp-api`, `spglib`. MP-20 ingests from plain CSVs of CIF strings, so it adds no dependency. If a symmetry cross-check is ever wanted, add it as warn-only logging that never writes a column — do not reintroduce `spglib` as a label source.
